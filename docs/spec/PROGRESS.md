...
- [x] v2.9.3 — security tooling refresh: SCA migration (Snyk → Dependabot/OSV) + CodeQL findings closure (2026-04-29)
  ...
  - [x] **PR #158** (`Version bump: patch`) — closes 5 CodeQL findings opened by the default-setup scan after enabling on 2026-04-28: 2× High "Incomplete multi-character sanitization" (`js/incomplete-multi-character-sanitization`) on `stripHtmlComments` and `stripHtmlTags` in `src/core/project-scanner/detectors/readme.js`, and 3× Medium "Workflow does not contain permissions" against the three jobs in `.github/workflows/ci.yml` (`test` matrix, `format-check`, `plugin-validate`). Sanitization fix uses the canonical do-while-until-stable pattern, extracted as a private `stripUntilStable(text, regex)` helper after `/review-changes` flagged the duplicated control flow. Permissions fix adds top-level `permissions: contents: read` (all three jobs need only read access). No regression test added — verified during plan-mode that no input distinguishes single-pass from looped output for the specific regex patterns used (`<[^>]+>` greedy and `<!--[\s\S]*?-->` non-greedy); the fix is defense-in-depth/static-analyzer appeasement. SECURITY.md's typosquat-alert section also refined after tracing Socket's chain to its actual source — Boris Cherny's deliberate [`bcherny/redirect-claude`](https://github.com/bcherny/redirect-claude) package, not an "abandoned" package as previously documented.
  - [x] Release group: 2 PRs (1 patch, 1 none). v2.9.2 → v2.9.3. No missing declarations.

- [x] v2.10.0 — Node 18 dropped + Dependabot misroute recovery (2026-04-29)
  - [x] **PR #167** (`Version bump: minor`) — drops Node 18 from the supported runtime matrix. Bumps `engines.node` from `>=18.0.0` to `>=20.0.0`, removes Node 18 from the CI test matrix in `.github/workflows/ci.yml` (`[18, 20, 22]` → `[20, 22]`), updates tech-stack mentions across CLAUDE.md, AGENTS.md, README.md (badge URL + alt text), `docs/guide/getting-started.md`, and `templates/specs/spec-md-library.md`. Triggered by Dependabot PRs blocked on Node-20-only features (`util.styleText` in inquirer 13, regex `v` flag in ora 9). Required UI step before push: removed `test (18)` from required-status-checks on both `develop-protection` and `main-protection` rulesets so CI on the PR could merge cleanly.
  - [x] **PR #168** (`Version bump: none`) — recovery PR. After PR #167 merged, all 7 Dependabot PRs (#160–#166) had been opened against `main` instead of `develop` because `.github/dependabot.yml` lacked `target-branch: develop`. Five (#160 prettier, #161 claude-code-action, #162 actions/cache, #163 vitest, #165 eslint) were squash-merged into main during a bulk-merge attempt before the misrouting was caught. This PR brings main's 5 commits onto develop via `git merge origin/main --no-ff` and adds `target-branch: develop` to both ecosystems in dependabot.yml so future Dependabot PRs target the correct branch. No code lost — the 5 dep updates are legitimate, just on the wrong branch; this PR reconciles develop's content with main's.
  - [x] **PR #169** (`Version bump: patch`) — bumps `ora` 8.2.0 → 9.4.0 and `inquirer` 12.11.1 → 13.4.2. Replaces the original Dependabot PRs #164 (ora) and #166 (inquirer), which were stuck due to the rebase tangle from being based on main with old `[18, 20, 22]` matrix. Cut from a fresh branch off post-recovery develop to bypass the rebase complications. Both bumps had been blocked solely by Node 18 incompatibility (now resolved by #167); CI passes cleanly on `[20, 22]`. CLAUDE.md flags inquirer as a breaking-change risk between majors — verified all 967 tests pass with the new majors.
...
- [x] v2.10.2 — Dogfood catch-up + workflow-ref scaffold fix + worktree gotcha doc (2026-04-30)
  - [x] **PR #174** (`Version bump: patch`) — bundled the v2.8.0 → v2.10.1 dogfood upgrade for this repo's own `.claude/` with a real scaffold bug fix surfaced during the upgrade run. Dogfood path: `worclaude upgrade` applied successfully (sandbox.network block self-tested ✓; new permissions for shell/WebFetch/WebSearch/built-in-Skills; observability hooks for UserPromptSubmit/InstructionsLoaded/SubagentStart/SubagentStop; 3 new obs-\*.cjs hooks; 3 helper scripts under `.claude/scripts/`; 6 commands and 1 skill cross-copied from v2.10.1 templates that had drifted ahead of the dogfood). Scaffold-fix path: `.claude/workflow-ref/` (transient upgrade-time conflict references) was missing from `src/core/scaffolder.js` `updateGitignore` entries — every user running `worclaude upgrade` had workflow-ref/ polluting their git status. Added the entry, updated 2 existing tests (`is idempotent` seed + `writes exactly N entries` count 10 → 11 + expected array), mirrored the rule into this repo's `.gitignore`. 27 of 33 files in workflow-ref/ were already byte-identical with `.claude/` — the maintainer had been keeping most of the dogfood current via direct edits.
