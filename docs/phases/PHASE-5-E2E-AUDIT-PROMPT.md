...
## Part A — Behavioral Coverage: Coding Principles Skill + CLAUDE.md Rules (Karpathy Option A)

**Goal:** Create a unified behavioral principles skill that consolidates scattered guidance into one always-referenceable file, and add Karpathy-derived rules to CLAUDE.md. This goes first so all subsequent audit steps validate the new content alongside everything else.

### Task A1: Create `templates/skills/universal/coding-principles.md`
...
cat templates/skills/universal/testing.md
cat templates/skills/universal/planning-with-files.md
cat templates/skills/universal/git-conventions.md
cat templates/agents/universal/code-simplifier.md
cat templates/commands/refactor-clean.md
```

Also read the Karpathy reference at `/home/sefa/SEFA/GIT/karpathy-cc/CLAUDE.md` for the original framing.
...
### Task A2: Register the new skill
...
### Task A3: Add Karpathy-derived Critical Rules to CLAUDE.md

Read the Critical Rules section in `templates/core/claude-md.md`. Add these if missing:
...
### Task A4: Reference from agent-routing
...
## Part B — Manifest Consistency Audit

**Goal:** Every file the scaffolder creates must be registered in the source manifests, and every manifest entry must have a corresponding template file. This now includes the coding-principles skill added in Part A.

### Task B1: Cross-reference all manifests against template files
...
### Task B2: Cross-reference CLAUDE.md template references
...
### Task B3: Cross-reference doctor checks against actual scaffold
...
### Task C3: Settings.json hook event validation
...
## Part D — CLAUDE.md Quality Audit
...
### Task D1: Line count verification

1. For each scaffolded test project from Part C, count CLAUDE.md lines
...
- Tech Stack
- Commands
- Skills (including the new coding-principles pointer from Part A)
- Session Protocol
- Critical Rules (including the 3 Karpathy-derived rules from Part A)
- Memory Architecture
- Learnings
- Gotchas

### Task D3: Critical Rules verification

Verify the Critical Rules section now covers all 12 rules (9 original + 3 Karpathy-derived from Part A). If Part A missed any, add them now.
...
1. **Part A** (coding-principles skill + CLAUDE.md Karpathy rules) — create new content first so subsequent audits validate it
2. **Part B** (manifest consistency) — catches broken references including the new skill
3. **Part C** (e2e scaffold test) — validates the full pipeline with all additions
4. **Part D** (CLAUDE.md quality) — verifies line count, sections, and critical rules after Part A additions
...
- **Fix as you go** — when you find a problem in Parts B-E, fix it immediately, don't just report it
...
- **If Part C scaffolding requires interactive input**, use the scaffolder programmatically or create a test harness that simulates choices. Check how existing tests handle this.
- **The coding-principles skill (Part A) must be concise** — under 80 lines. It's a reference card that consolidates scattered content, not new documentation.
